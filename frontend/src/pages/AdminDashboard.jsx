...
import { Users, BookOpen, MessageSquare, Upload, Plus, FileVideo, HardDrive, Trash2, Edit } from 'lucide-react';

const AdminDashboard = () => {
    const [activeTab, setActiveTab] = useState('overview'); // overview, courses, modules, lessons

    // Data States
    const [stats, setStats] = useState({ totalUsers: 0, newUsersToday: 0, totalCourses: 0, totalLessons: 0, openQueries: 0 });
    ...
    const fetchOverviewData = async () => {
        try {
            const [statsRes, queriesRes] = await Promise.all([
                ...
                fetch('http://localhost:5000/api/admin/queries')
            ]);
    ...
    const fetchCourses = async () => {
        try {
            const res = await fetch('http://localhost:5000/api/learning/courses');
    ...
    const fetchModules = async (courseId) => {
        try {
            const res = await fetch(`http://localhost:5000/api/learning/courses/${courseId}/modules`);
            const data = await res.json();
    ...
    const fetchAllModules = async () => {
        ...
        for (let c of courses) {
            try {
                const res = await fetch(`http://localhost:5000/api/learning/courses/${c._id}/modules`);
                const data = await res.json();
                if (data.status === 'success') {
                    allMods = [...allMods, ...data.data.map(m => ({ ...m, courseTitle: c.title }))];
                }
            } catch (e) { }
        }
        setModules(allMods);
                ...
                data.data.forEach(course => {
                    course.modules.forEach(mod => {
                        mod.lessons.forEach(l => {
                            allLessons.push({ ...l, moduleTitle: mod.title, courseTitle: course.title });
                        });
                    });
                });
    ...
    const deleteCourse = async (id) => {
        if (!window.confirm("Delete this Course completely? (Cascade delete not enforced in UI test)")) return;
        try {
            const res = await fetch(`http://localhost:5000/api/admin/courses/${id}`, { method: 'DELETE' });
    ...
    const createModule = async (e) => {
        ...
        } catch (error) { alert('Error creating module.'); }
    };

    const deleteModule = async (id) => {
        if (!window.confirm("Delete this Module?")) return;
        try {
            const res = await fetch(`http://localhost:5000/api/admin/modules/${id}`, { method: 'DELETE' });
            if (res.ok) fetchAllModules();
        } catch (e) { alert("Failed to delete."); }
    };

    const deleteLesson = async (id) => {
        if (!window.confirm("Delete this Lesson? The file will also be destroyed in Cloudinary.")) return;
        try {
            const res = await fetch(`http://localhost:5000/api/admin/lessons/${id}`, { method: 'DELETE' });
            if (res.ok) {
                alert('Deleted');
                fetchAllLessons();
            }
    ...
    if (loading && activeTab === 'overview') return <div className="p-8 text-center text-gray-500">Loading Dashboard...</div>;
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            {activeTab === 'overview' && (
                <div className="space-y-8 animate-in fade-in duration-300">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {[
                            { label: 'Total Users', value: stats.totalUsers, icon: <Users size={24} />, color: 'text-blue-600', bg: 'bg-blue-50' },
                            { label: 'Total Courses', value: stats.totalCourses, icon: <BookOpen size={24} />, color: 'text-purple-600', bg: 'bg-purple-50' },
                            { label: 'Total Lessons', value: stats.totalLessons, icon: <FileVideo size={24} />, color: 'text-emerald-600', bg: 'bg-emerald-50' },
                            { label: 'Open Queries', value: stats.openQueries, icon: <MessageSquare size={24} />, color: 'text-amber-600', bg: 'bg-amber-50' }
                        ].map((stat, i) => (
            ...
            {activeTab === 'courses' && (
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 animate-in fade-in duration-300">
                    ...
                    <div className="divide-y divide-gray-100 border border-gray-100 rounded-xl overflow-hidden">
                        {courses.length === 0 ? <p className="p-4 text-gray-500 text-sm">No courses found.</p> : courses.map(c => (
                            <div key={c._id} className="flex items-center justify-between p-4 hover:bg-gray-50">
                                <span className="font-semibold text-gray-800">{c.title}</span>
                                <div className="flex gap-3">
                                    <span className="text-xs bg-gray-100 px-2 py-1 rounded text-gray-500 font-mono">{c._id}</span>
                                    <button onClick={() => deleteCourse(c._id)} className="text-red-500 hover:text-red-700 p-1"><Trash2 size={18} /></button>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {activeTab === 'modules' && (
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 animate-in fade-in duration-300">
                    <div className="flex flex-col lg:flex-row justify-between lg:items-center mb-6 gap-4">
                        <h2 className="text-xl font-bold text-gray-900 border-l-4 border-purple-500 pl-3">Manage Modules</h2>
                        <form onSubmit={createModule} className="flex flex-wrap gap-2">
                            <select value={newModuleData.courseId} onChange={e => setNewModuleData({ ...newModuleData, courseId: e.target.value })} className="px-3 py-2 border border-gray-300 rounded-lg text-sm" required>
                                <option value="" disabled>Select Parent Course</option>
                                {courses.map(c => <option key={c._id} value={c._id}>{c.title}</option>)}
                            </select>
                            <input type="text" value={newModuleData.title} onChange={e => setNewModuleData({ ...newModuleData, title: e.target.value })} placeholder="Module Title" className="px-3 py-2 border border-gray-300 rounded-lg text-sm w-48" required />
                            <input type="number" value={newModuleData.order} onChange={e => setNewModuleData({ ...newModuleData, order: e.target.value })} placeholder="Order" className="px-3 py-2 border border-gray-300 rounded-lg text-sm w-20" required min="1" />
                            <button type="submit" className="bg-purple-600 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-1 hover:bg-purple-700"><Plus size={16} /> Add Module</button>
                        </form>
                    </div>
                    <div className="divide-y divide-gray-100 border border-gray-100 rounded-xl overflow-hidden">
                        {modules.length === 0 ? <p className="p-4 text-gray-500 text-sm">No modules found or loading...</p> : modules.map(m => (
                            <div key={m._id} className="flex items-center justify-between p-4 hover:bg-gray-50">
                                <div>
                                    <span className="font-semibold text-gray-800">{m.title}</span>
                                    <p className="text-xs text-purple-600 font-medium mt-0.5">Parent Course: {m.courseTitle || m.courseId}</p>
                                </div>
                                <div className="flex gap-3 items-center">
                                    <span className="text-xs bg-gray-100 px-2 py-1 rounded text-gray-500 font-mono">Order: {m.order}</span>
                                    <button onClick={() => deleteModule(m._id)} className="text-red-500 hover:text-red-700 p-1"><Trash2 size={18} /></button>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {activeTab === 'lessons' && (
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 animate-in fade-in duration-300">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-xl font-bold text-gray-900 border-l-4 border-emerald-500 pl-3">Manage Lessons</h2>
                        <button onClick={() => setActiveTab('overview')} className="text-emerald-600 text-sm font-medium hover:underline">Go to Overview to Upload (+)</button>
                    </div>
                    {loading ? <p className="p-4 text-gray-500 text-sm">Organizing huge classroom... please wait.</p> : (
                        <div className="divide-y divide-gray-100 border border-gray-100 rounded-xl overflow-hidden">
                            {lessons.length === 0 ? <p className="p-4 text-gray-500 text-sm">No lessons found.</p> : lessons.map(l => (
                                <div key={l._id} className="flex items-center justify-between p-4 hover:bg-gray-50">
                                    <div className="flex items-center gap-4">
                                        {l.mediaType === 'video' ? <FileVideo className="text-emerald-500 w-8 h-8" /> : <HardDrive className="text-emerald-500 w-8 h-8" />}
                                        <div>
                                            <a href={l.mediaUrl} target="_blank" rel="noreferrer" className="font-semibold text-gray-800 hover:text-emerald-600 transition-colors">{l.title}</a>
                                            <p className="text-xs text-gray-500 font-medium mt-0.5">
                                                {l.courseTitle} &rarr; {l.moduleTitle}  <span className="ml-2 bg-gray-200 px-1 py-0.5 rounded uppercase">{l.type}</span>
                                            </p>
                                        </div>
                                    </div>
                                    <button onClick={() => deleteLesson(l._id || l.id)} className="text-red-500 hover:text-red-700 p-2 border border-transparent hover:border-red-100 rounded-lg"><Trash2 size={18} /></button>
                                </div>
                            ))}
                        </div>
                    )}
